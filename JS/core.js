...
class UI{
    ...
    static validateInputs(){
        ...
        var licensePlateRegex = /^(?:[A-Z]{3} [0-9]{3} [A-Z]{1})$/;
        ...
        if(!licensePlateRegex.test(licensePlate)){
            UI.showAlert('License Plate must be like NNN 123 L ','danger');
            return false;
        }
        if(licensePlateRegex==licensePlateRegex){
            UI.showAlert('License Plate is already registered under a different name ','danger');
            return false;
        }
        return true;       
